...
export class NotesFormComponent implements OnInit {
  ...
  // ➕ Ici on transforme le PDF en blob pour l'envoyer
  ...
  this.notesService.uploadDocument(formData).subscribe({
    next: () => this.toaster.success('Relevé envoyé dans les documents 📤'),
    error: (err) => {
      this.toaster.error('Erreur lors de l\'envoi du relevé 📄');
      console.error(err);
    }
  });
}
