...
@Component({
  ...
  styleUrl: './attestation.css'
})
export class Attestation implements OnInit{
  ...
  async telechargerPDF(elementId: string, filename: string): Promise<void> {
  ...
  pdf.save(`${filename}-${this.etudiant.nom}-${this.etudiant.num_etudiant}.pdf`);
}
